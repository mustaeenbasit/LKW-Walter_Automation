...
public class Teams_26078  extends SugarTest {
	...
	/**
	 *  Combine team id search condition with another search condition (any)
	 * 
	 * @throws Exception
	 */
	@Test
	public void Teams_26078_execute() throws Exception {
		...
		VoodooControl searchConditionCtrl = new VoodooControl("input", "css", "#team_name_advanced_type[value='any']");
		VoodooControl documentNameCtrl = new VoodooControl("input", "id", "document_name_advanced");
		VoodooControl listItemRowCtrl = new VoodooControl("td", "css", "#MassUpdate tr.oddListRowS1  td:nth-child(4)");
		for (int i = 0; i < documentsData.size(); i++) {
			// input document name
			documentNameCtrl.set(documentsData.get(i).get("documentName"));
			// input teams id (eg. qauser,Global)
			if(i == 2){
				teamCtrl.set(teamsData.get(1).get("teamName"));
			}else{
			...
			listItemRowCtrl.assertContains(documentsData.get(i).get("documentName"), true);
		}
		VoodooUtils.focusDefault();

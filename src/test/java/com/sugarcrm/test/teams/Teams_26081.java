...
public class Teams_26081 extends SugarTest {
	...
	/**
	 * Combine team id search condition with another search condition (exact)
	 * 
	 * @throws Exception
	 */
	@Test
	public void Teams_26081_execute() throws Exception {
		...
		String docToSearch =  documentsData.get(1).get("documentName").substring(0,  documentsData.get(1).get("documentName").length()-1);
		nameBasic.set(docToSearch);
		basicSearch.click();
		VoodooUtils.focusDefault();

		sugar.documents.listView.checkRecord(2);
		sugar.documents.listView.checkRecord(3);
		...
		VoodooControl searchConditionCtrl = new VoodooControl("input", "css", "#team_name_advanced_type[value='exact']");
		...
		for (int i = 0; i < documentsData.size()-1; i++) {
			...
			// Select a radio button below the team id (i.e. exact)

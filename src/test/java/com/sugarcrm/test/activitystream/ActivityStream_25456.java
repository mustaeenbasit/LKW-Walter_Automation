...
public class ActivityStream_25456 extends SugarTest {
	...
	/**
	 * Correct Activity Stream messages when create a call that is related to RLI or QLI.
	 *
	 * @throws Exception
	 */
	@Test
	public void ActivityStream_25456_execute() throws Exception {
		...
		for (StandardRecord rec : records) {
			...
			sugar.calls.createDrawer.getEditField("name").set(ds.get(i).get("name"));
			sugar.calls.createDrawer.getEditField("relatedToParentType").set(ds.get(i).get("related"));
			if(i == 0){
			...
			// the call create and link are happened at same time, so these messages' position are kind random in activity stream
			...
			// show Cl as module icon for create call and link user to call message, show RL/QL as module icon for link call to RLI/QLI message
			for (int j = 1; j <= 3; j++) {
				// TODO: VOOD-977

...
public class portalCrudCase extends SugarTestApi {
	...
	@Test
	/**
	 * Test Case portalCrudCase: Verifies the soap calls for Create, Read, Update and Delete Cases
	 * 
	...
	public void main() throws JSONException, ParseException,
		...
		VoodooUtils.voodoo.log.info("SOAP Create CAse Request " + newRequestBodyString);

		xml = given().request().contentType("text/xml; charset=UTF-8;")
				.body(newRequestBodyString).when().post("soap.php").andReturn()
				.asString();
		VoodooUtils.voodoo.log.info("SOAP Create CaseResponse " + xml);
		...
		content = new Scanner(
				new File(
						"src/test/resources/apidata/soap/common/portalGetEntryCasesSingle.xml"))
				.useDelimiter("\\Z").next();

		newRequestBodyString = content.replace("mySessionID", sessionID);
		String getRecord = newRequestBodyString.replace("myCaseID", recordID);
		...
		content = new Scanner(
				new File(
						"src/test/resources/apidata/soap/common/portalSetEntryUpdateCases.xml"))
				.useDelimiter("\\Z").next();

		newRequestBodyString = content.replace("mySessionID", sessionID);
		content = newRequestBodyString.replace("myCaseID", recordID);
		VoodooUtils.voodoo.log.info("SOAP Request " + content);
		...
		assertTrue(xml.contains("MYNEW case Updated Again"));
		assertFalse(xml.contains("My SOAP Test Case XYZ"));

		// Verify the created Case can be deleted using the ID
		content = new Scanner(
				new File(
						"src/test/resources/apidata/soap/common/portalSetEntryDeleteCases.xml"))
				.useDelimiter("\\Z").next();
		...
		assertFalse(xml.contains("MYNEW case Updated Again"));
		assertFalse(xml.contains("My SOAP Test Case XYZ"));

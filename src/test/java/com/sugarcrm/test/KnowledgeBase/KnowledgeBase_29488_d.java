...
public class KnowledgeBase_29488_d extends SugarTest {
	public void setup() throws Exception {
		...
		// Creating a KB record with past expiration date and blank published date
		...
		sugar().knowledgeBase.createDrawer.getEditField("date_expiration").set(DateTime.now().minusDays(2).toString("MM/dd/yyyy"));
	 ...
	 * KnowledgeBase_29488_d : Verify the KB with blank published date and past expiration date in Cases sub panel gives
	 * correct results when edited and status changed to Approved
	 * @throws Exception
	 */
	@Test
	public void KnowledgeBase_29488_d_execute() throws Exception {
		...
		sugar().alerts.getWarning().assertContains(fs.get("warningMessage"), true);
		...
		VoodooControl dateCalendar = new VoodooControl("div", "css", ".datepicker.dropdown-menu");

		// Asserting the publish date calendar is visible and setting future date from calendar
		publishDate.scrollIntoViewIfNeeded(true);
		dateCalendar.assertVisible(true);
		dateCalendar.getChildElement("td", "css", ".day.new").click();
		kbSubpanel.saveAction(1);

		// Asserting the Error message and getting the date value entered for Publish date
		sugar().alerts.getError().assertVisible(true);
		sugar().alerts.getError().assertContains(fs.get("errorMessage"), true);
		VoodooUtils.waitForReady();

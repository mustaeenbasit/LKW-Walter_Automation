...
public class KnowledgeBase_29488_b extends SugarTest {
	public void setup() throws Exception {
		...
		// Creating a KB record with past publish date and blank expiration date
		...
		sugar().knowledgeBase.createDrawer.getEditField("date_publish").set(DateTime.now().minusDays(2).toString("MM/dd/yyyy"));
	 ...
	 * KnowledgeBase_29488_b : Verify the KB with past published date and blank expiration date in Cases sub panel gives
	 * correct results when edited and status changed to Approved
	 * @throws Exception
	 */
	@Test
	public void KnowledgeBase_29488_b_execute() throws Exception {
		...
		// TODO: VOOD-1489
		VoodooDate publishDate = new VoodooDate("input", "css", "tr:nth-child(1) .fld_active_date div input");

		// Setting future date in publish date column
		String newPublishDate = DateTime.now().plusMonths(1).toString("MM/dd/yyyy");
		publishDate.scrollIntoViewIfNeeded(false);
		publishDate.set(newPublishDate);
		kbSubpanel.saveAction(1);

		// Asserting the publish date and status field after save
		// TODO: VOOD-1489
		new VoodooControl("div", "css", "tr:nth-child(1) .fld_active_date div").assertEquals(newPublishDate, true);

...
public class Roles_21296 extends SugarTest {
	...
	public void setup() throws Exception {
		...
		// Set Billing Street as None
		new VoodooControl("div", "id", "billing_addresslink").click();
		new VoodooControl("select", "id", "flc_guidbilling_address").set(customFS.get("field_permission"));
	...
	/**
	 * Access Type "Admin", Field permissions set to "None" & "Not Set" on Accounts (group field)
	 * @throws Exception
	 */
	@Test
	public void Roles_21296_execute() throws Exception {
		VoodooUtils.voodoo.log.info("Running " + testName + "...");

		// Go to Accounts record listView
		sugar().accounts.navToListView();

		// Verify that QAuser can view the name of "PrivateAccount" on the listview.
		sugar().accounts.listView.verifyField(1, "name", customFS.get("accountName"));

		// Verify that QAuser is shown "No Access" on City and Billing on ListView.
		// TODO: VOOD-1445
		new VoodooControl("span", "css", ".noaccess.fld_billing_address_city").assertEquals(customFS.get("noAccess"), true);
		new VoodooControl("span", "css", ".noaccess.fld_billing_address_country").assertEquals(customFS.get("noAccess"), true);

		// Navigate to Account Record View
		sugar().accounts.listView.clickRecord(1);
		sugar().accounts.recordView.showMore();

		// Verify that Billing Street information is not visible.
		// TODO: VOOD-1445
		new VoodooControl("span", "css", ".noaccess.fld_billing_address span").assertEquals(customFS.get("noAccess"), true);
		sugar().accounts.recordView.edit();

...
public class Leads_30217 extends SugarTest {
	...
	/**
	 * Verify that Uncheck "Move related activities to the contact record" won't move activities In Lead Convert (no email address)
	 * @throws Exception
	 */
	@Test
	public void Leads_30217_execute() throws Exception {
		...
		// Un-checking the 'Move related activities to the contact record' checkbox
		new VoodooControl("input", "css", ".fld_transfer_activities input").click();
		...
		// Verify that there is no records moved from Calls,Meetings,Tasks,Notes and Emails subpanel on Leads Records view.
		sugar().leads.recordView.setRelatedSubpanelFilter(callsPlural);
		Assert.assertTrue("Records moved from Calls subpanel", callsSubpanel.countRows() == 1);
		sugar().leads.recordView.setRelatedSubpanelFilter(meetingsPlural);
		Assert.assertTrue("Records moved from Meetings subpanel", meetingsSubpanel.countRows() == 1);
		sugar().leads.recordView.setRelatedSubpanelFilter(tasksPlural);
		Assert.assertTrue("Records moved from Tasks subpanel", taskSubpanel.countRows() == 1);
		sugar().leads.recordView.setRelatedSubpanelFilter(notesPlural);
		Assert.assertTrue("Records moved from Notes subpanel", noteSubpanel.countRows() == 1);
		sugar().leads.recordView.setRelatedSubpanelFilter(emailsPlural);
		Assert.assertTrue("Records moved from Emails subpanel", emailsSubpanel.countRows() == 2);
		...
		Assert.assertTrue("Records moved to Calls subpanel of Contact", contactCallsSubpanel.countRows() == 0);
		sugar().contacts.recordView.setRelatedSubpanelFilter(meetingsPlural);
		Assert.assertTrue("Records moved to Meetings subpanel of Contact", contactMeetSubpanel.countRows() == 0);
		sugar().contacts.recordView.setRelatedSubpanelFilter(tasksPlural);
		Assert.assertTrue("Records moved to Tasks subpanel of Contact", contactTaskSubpanel.countRows() == 0);
		sugar().contacts.recordView.setRelatedSubpanelFilter(notesPlural);
		Assert.assertTrue("Records moved to Notes subpanel of Contact", contactNoteSubpanel.countRows() == 0);
		...
		Assert.assertTrue("Records moved to Emails subpanel of Contact", contactEmailSubpanel.countRows() == 0);
		...
		// Verify records present in Leads and Contact subpanel on accounts record view
		...
		// Verify on Email detail view Lead record appears in Leads subpanel
		sugar().leads.navToListView();
		sugar().leads.listView.clickRecord(1);
		sugar().leads.recordView.setRelatedSubpanelFilter(emailsPlural);

		// TODO: VOOD-972 - Not working methods in BWC subpanels: verify, assertContains, create
		new VoodooControl("a", "css", ".layout_Emails .single .fld_name a").click();
		VoodooUtils.focusFrame("bwc-frame");

		// TODO: VOOD-792 - Need lib support to handle or access email records
		new VoodooControl("a", "css", "#list_subpanel_leads tr.oddListRowS1 a").assertContains(leadRec.getRecordIdentifier(), true);

		// Verify on Email detail view no record appears in Contact and Email Contact subpanel
		String nodata = customData.get("noData");
		new VoodooControl("div", "id", "list_subpanel_contacts").assertContains(nodata, true);

...
public class GlobalSearch_20097 extends SugarTest {
	public void setup() throws Exception {
		sugar().login();
	}

	/**
	 * Check the data type of fields that support full text search function
	 * @throws Exception
	 */
	@Test
	public void GlobalSearch_20097_execute() throws Exception {
		...
		for(int i = 0; i < customDS.size(); i++) {
			// If the field data type is: TextField ,TextArea, URL, Integer or Phone
			dataTypeDropdownCtrl.set(customDS.get(i).get("dataType"));
			VoodooUtils.waitForReady();

			// Verify that Full Text Searchable drop down in the field edit page
			ftsFieldCtrl.assertVisible(true);
		}
